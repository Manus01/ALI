...
export default function BrandOnboarding() {
    ...
    const handleLogoChange = (e) => {
        ...
        // Senior Guardrail: Ensure high-quality formats with transparency support
    ...
    const handleSelectStyle = async (style) => {
        ...
        try {
            // A. Upload Logo to Firebase Storage if provided
            ...
            // B. Complete Onboarding with DNA + Logo Reference
            await api.post('/onboarding/complete', {
                brand_dna: {
                    ...dna,
                    selected_style: style.id,
                    logo_url: finalLogoUrl // Injected for future watermarking
                }
            });

            await refreshProfile(); // Sync Sidebar
            navigate('/dashboard');
        } catch (err) {
            ...
            <div className="max-w-4xl w-full animate-fade-in">
                ...
                {step === 'input' && (
                    <div className="glass-panel p-10 bg-white rounded-3xl shadow-xl border border-slate-100">
                        ...
                        <p className="text-slate-500 mb-10 max-w-lg mx-auto text-center font-medium">
                            {useDescription
                                ? "Describe your vision and we'll craft your brand DNA from scratch."
                                : "Give us your URL and our AI will extract your brand's core essence."}
                        </p>

                        <div className="max-w-lg mx-auto space-y-8">
                            {/* 1.1 Hybrid Source Logic */}
                            <div className="space-y-4">
                                ...
                                {!useDescription ? (
                                    ...
                                    <div className="group relative">
                                        <textarea
                                            placeholder="Describe your products, niche, and the 'vibe' of your company..."
                                            className="w-full p-5 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-primary/10 focus:border-primary outline-none text-lg min-h-[160px] transition-all"
                                            value={description}
                                ...
                                <div className="relative group border-2 border-dashed border-slate-200 rounded-2xl p-6 hover:border-primary transition-all bg-slate-50/50">
                                    ...
                                    {logoPreview ? (
                                        ...
                                        <div className="text-center py-2">
                                            ...
                                            <p className="text-xs text-slate-500 font-bold tracking-tight">Upload Logo for watermarking</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* 1.3 Target Markets */}
                            <div className="space-y-4 pt-4 border-t border-slate-50">
                                <label className="block text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Target Markets</label>
                                    ...
                                    {['Cyprus', 'Germany', 'USA', 'UK', 'Greece'].map(c => (
                                        <button
                                            ...
                                            className={`px-5 py-2.5 rounded-full border text-sm font-bold transition-all flex items-center gap-2
                                                ${countries.includes(c) ? 'bg-primary text-white border-primary shadow-lg shadow-blue-500/20' : 'bg-white text-slate-400 border-slate-200 hover:border-primary/50'}`}
                                        >
                                            <FaGlobe className={countries.includes(c) ? "opacity-100" : "opacity-50"} /> {c}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                className="w-full bg-gradient-to-r from-primary to-blue-700 text-white py-5 rounded-2xl font-black text-lg shadow-xl shadow-blue-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-30 disabled:grayscale uppercase tracking-widest"
                            >
                                Build My DNA
                            </button>
                        </div>
                ...
                {step === 'loading' && (
                    <div className="flex flex-col items-center py-32 text-center">
                        ...
                        <h3 className="text-3xl font-black text-slate-800 mb-3 tracking-tight">Decoding Brand DNA...</h3>
                        <p className="text-slate-400 max-w-md mx-auto leading-relaxed">
                            Orchestrating visual assets and cultural nuances for {countries.length > 0 ? countries.join(', ') : 'Global Markets'}.
                        </p>
                    </div>
                )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            {dna.visual_styles?.map((style, idx) => (
                                <button
                                    ...
                                    disabled={isSaving}
                                    className="group relative p-10 bg-white border-2 border-slate-100 rounded-[2rem] hover:border-primary hover:shadow-2xl hover:-translate-y-2 transition-all text-left flex flex-col h-full overflow-hidden"
                                >
